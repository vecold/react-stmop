...
type funcType = {
  (props: funcProps): void;
};

type tempFunc = { (func: funcType): funcType };

const subscriptionMap = new Map<string, StompSubscription>();
const subscriptionCallbackMap = new Map<
  string,
...
const subscriptionFailedMap: Map<string, { func: any; args: any }[]> =
  new Map();
...
const stompSubscribe = (destination: string) => {
  const subscription = stompClient?.subscribe(destination, (message: any) => {
    ...
    subscriptionCallbackMap
      .get(destination)
      ?.forEach((fuc) => fuc.current(payload));
  });
  return subscription;
};
...
export const initWebSocketUrl = (url: string) => {
  webSocketUrl = url;
};
...
export function unsubscribe({
  ...
  failedArr = failedArr.filter(
    (item) => item.args[0].msgCallback !== msgCallback
  );
...
export const send = warpFuncWithConnectLogic(
  (props: { message?: string; destination: string }) => {
    const { destination, message } = props;
    // 检测链接是否正常
    stompClient?.publish({
      destination,
